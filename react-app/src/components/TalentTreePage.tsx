...
export function TalentTreePage({
  ...
  useEffect(() => {
    if (!metaProgress?.specializations.length) {
      setActiveSpec(null);
      return;
    }

    setActiveSpec((current) =>
      current && metaProgress.specializations.some((specialization) => specialization.id === current)
        ? current
        : metaProgress.specializations[0]?.id ?? null,
    );
  }, [metaProgress]);
  ...
  const selectedSpecialization =
    metaProgress.specializations.find((specialization) => specialization.id === activeSpec) ??
    metaProgress.specializations[0] ??
    null;
  const visibleTalents = selectedSpecialization ? specMap[selectedSpecialization.id] ?? [] : [];
      ...
      <div className="specialization-tabs">
        {metaProgress.specializations.map((specialization) => (
          <button
            ...
            type="button"
            className={`specialization-tab${activeSpec === specialization.id ? " active" : ""}`}
      ...
      >
        ...
        {visibleTalents.map((talent) => (
          <article
            ...
            }${talent.id.endsWith("capstone") ? " capstone" : ""}${getTalentElementClass(
              getSelectedElement(talent),
            )}`}
            style={{
